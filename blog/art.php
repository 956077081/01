...
if(!empty($_POST)){
  ...
  $comm['content']= htmlspecialchars(trim($_POST['content']));//防止XSS攻击  防止直接转为HTML 与 HTML代码连接
...
//按buptime的  升序行排列
$sql="select user.nick,pubtime,content,pripic from  comment,user where art_id=$art_id and comment.user_id = user.user_id order by pubtime ASC ";
